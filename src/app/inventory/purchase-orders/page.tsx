...
// ============================================================================
// INVENTORY MANAGER — PURCHASE ORDERS PAGE
// ============================================================================
...
export default function InventoryPurchaseOrdersPage() {
  ...
  useEffect(() => {
    if (storeId) fetchOrders(storeId, true);
  ...
  useEffect(() => {
    if (storeId) fetchSuppliers(storeId);
  ...
  const handleReceiveItems = async (poId: string, receivedItems: ReceiveItemRequest[]): Promise<boolean> => {
    if (!storeId) return false;
    const toastId = toast.loading("Receiving items...");
    const success = await receiveItems(storeId, poId, receivedItems);
    if (success) {
  ...
  const handleFiltersChange = useCallback(
    (newFilters: Partial<PurchaseOrderFilters>) => setFilters(newFilters),
    [setFilters]
  );
  ...
  const handleLimitChange = useCallback(
    (limit: number) => setPagination({ limit, page: 1 }),
    [setPagination]
  );

  const handleSelectAll = useCallback(
    (ids: string[]) => setSelectedOrderIds(ids),
    [setSelectedOrderIds]
  );
